...
做法是优化建图，对于每个完全图建立若干辅助点，原图点与辅助点匹配表示在该组中，只需去除辅助点与辅助点匹配的贡献即可。
...
$$
f_i = \sum_j a_{i+j} \dbinom{k - 1}{i} \bmod 2
$$

由库默尔定理分析，有值的 $i$ 相当于 $i + (k - 1 - i)$ 在二进制下不进位，
即 $i$ 是 $k - 1$ 子集。
...
考虑主元法，每个方程有 $2k+1$ 个变量，因此可以选定前 $2k$ 个变量为主元，就能通过 $n - 2k$ 个方程线性表示出其他 $n - 2k$ 个变量。然后用剩下 $2k + 1$ 个方程解出主元即可。复杂度 $O((2k)^3)$。